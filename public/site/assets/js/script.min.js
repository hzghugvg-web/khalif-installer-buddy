(() => {
  ...
  async function loadIPATools() {
    ...
    grid.innerHTML = '<div class="library-state">Загрузка библиотеки…</div>';
    try {
      const response = await fetch("/api/public/library");
      if (!response.ok) throw new Error("Не удалось загрузить библиотеку");
      const payload = await response.json();
      ipaToolsData = Array.isArray(payload.apps) ? payload.apps.map(app => ({
        name: app.name,
        iconURL: app.icon_url,
        developerName: app.version || app.description || "KINST",
        downloadURL: app.install_url,
        category: app.category || "tweak"
      })) : [];
      filteredIPATools = ipaToolsData;
      ipaToolsVisible = 0;
      ipaToolsLoaded = true;
      renderIPATools(true);
      if (!ipaToolsData.length) grid.innerHTML = '<div class="library-state"><strong>Библиотека пока пуста</strong><span>Скоро здесь появятся приложения.</span></div>';
    } catch (error) {
      console.error("KINST LIBRARY ERROR:", error);
      grid.innerHTML = '<div class="library-state">Не удалось загрузить библиотеку.</div>';
    } finally {
  ...
  function renderIPATools(reset = false) {
    ...
    items.forEach(app => {
      ...
      developer.textContent = app.developerName || "KINST";
