(() => {
  ...
  function detectCategory(app, source) {
    const text = `${app.name || ""} ${app.bundleID || ""} ${app.bundleIdentifier || ""} ${app.localizedDescription || ""}`.toLowerCase();

    if (source === "roy" && app.category) return app.category;
    ...
    if (/music|audio|radio|piano|guitar|drum|vocal|song|sing|beat|eeve|spotify|player|sampler|dj|koala|speak|mix|davinci|garageband/.test(text)) return "music";
    if (/photo|video|studio|vsco|beauty|design|vn|vlog|face|procreate|camera|color|art|kinemaster|pic|picture|adobe|pixel|edit|editor|filter|tv|iptv|oldroll|eraser|capcut|reel/.test(text)) return "photo-video";
    return "tweak";
  }

  function normalizeRoy(app) {
    return {
      name: app.name,
      iconURL: app.iconURL,
      developerName: app.developerName || "AppleJr",
      downloadURL: app.downloadURL,
      category: detectCategory(app, "roy"),
      versionDate: app.versionDate || "",
      key: (app.name || "").toLowerCase()
    };
  }

  function normalizeAppTesters(app) {
    return {
      name: app.name,
      iconURL: app.iconURL || app.icon,
      developerName: "AppTesters",
      downloadURL: app.downloadURL || app.down,
      category: detectCategory(app, "apptesters"),
      versionDate: app.versionDate || "",
      key: (app.bundleIdentifier || app.name || "").toLowerCase()
    };
  }
  ...
  function dedupeLatest(items) {
    ...
    items.forEach(item => {
      ...
      if (!current || Date.parse(item.versionDate) > Date.parse(current.versionDate)) {
  ...
  async function loadIPATools() {
    ...
    try {
      const [royResponse, appTestersResponse] = await Promise.all([
        fetch(`https://raw.githubusercontent.com/royilkom-alt/toolsrepo/refs/heads/main/tools.json?ts=${Date.now()}`),
        fetch(`https://repository.apptesters.org/apps.json?ts=${Date.now()}`)
      ]);

      if (!royResponse.ok || !appTestersResponse.ok) throw new Error("Fetch failed");

      const royJson = await royResponse.json();
      const appTestersJson = await appTestersResponse.json();

      const royApps = Array.isArray(royJson.apps) ? royJson.apps.map(normalizeRoy) : [];
      const appTesterApps = Array.isArray(appTestersJson.apps) ? appTestersJson.apps.map(normalizeAppTesters) : [];

      ipaToolsData = shuffleArray(dedupeLatest([...royApps, ...appTesterApps]));
      filteredIPATools = ipaToolsData;
      ipaToolsVisible = 0;
      ipaToolsLoaded = true;
      renderIPATools(true);
    } catch (error) {
      console.error("IPA TOOLS ERROR:", error);
      grid.innerHTML = '<div style="grid-column:1/-1;padding:26px;text-align:center;color:#7d8590;font-size:11px">❌ Failed to load IPA Tools</div>';
    } finally {
  ...
  function renderIPATools(reset = false) {
    ...
    items.forEach(app => {
      ...
      developer.textContent = app.developerName || "AppleJr";
    ...
    loadMore.style.display = ipaToolsVisible < filteredIPATools.length ? "block" : "none";
  }

  document.getElementById("toolsLoadMore")?.addEventListener("click", () => renderIPATools(false));
  ...
  $$(".tool-cat").forEach(button => {
    button.addEventListener("click", () => {
      ...
      filteredIPATools = category === "all" ? ipaToolsData : ipaToolsData.filter(app => app.category === category);
      renderIPATools(true);
    });
  });
  ...
  document.getElementById("toolsSearch")?.addEventListener("input", event => {
    ...
    filteredIPATools = query
      ? ipaToolsData.filter(app => {
          const haystack = `${app.name} ${app.developerName} ${app.category}`.toLowerCase();
